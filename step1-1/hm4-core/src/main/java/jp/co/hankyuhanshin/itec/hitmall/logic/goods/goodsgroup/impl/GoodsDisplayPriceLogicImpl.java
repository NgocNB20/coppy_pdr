 ...
 */
@Component
public class GoodsDisplayPriceLogicImpl extends AbstractShopLogic implements GoodsDisplayPriceLogic {

    // 2023-renew AddNo5 from here
    /**
     * ログ<br/>
     */
    private static final Logger LOGGER = LoggerFactory.getLogger(GoodsDisplayPriceLogicImpl.class);
    ...
    public Map<Key, Object> create(GoodsGroupDto goodsGroupDto, boolean isSaleTimeCheck) {
        ...
        goodsDtoList = goodsDtoList.stream()
                                   ...
                                   // 販売制御区分　≠　「2：取扱中止」
                                   // 販売可否判定結果 = 「1：販売可」
                                   .filter(goodsDto -> !HTypeSaleControlType.DISCONTINUED.equals(
                                                   goodsDto.getGoodsEntity().getSaleControl()))
    ...
    public Map<Key, Object> create(List<GoodsDto> goodsList,
        ...
        // 2023-renew AddNo5 from here
        // 商品グループMobile用価格判定マップ（最高値、最安値、値引き前最安値を判定する）
        Map<PricesKey, BigDecimal> pricesMb = createPrices();
        // 商品グループMobile用表示価格判定マップ（値引き前価格の価格帯有無、値引き前価格有無を判定する）
        Map<DisplayPriceManagerKey, Boolean> displayPriceManagerMb = createDisplayPriceManager();
        // 商品グループMobile用金額変更判定マップ（値下げ、値上げ、変更なしを判定する）
        Map<PriceManagerKey, Integer> priceManagerMb = createPriceManager();
        // 2023-renew AddNo5 to here
        ...
        for (GoodsDto goods : goodsList) {
            ...
            calcGoodsDisplayPrice(prices, displayPriceManager, priceManager, entity, false);

            // 商品表示価格Mobile関連情報 （セール価格用）（最高値、最安値、商品表示価格、商品表示値引き前価格）を算出する
            calcGoodsDisplayPrice(pricesMb, displayPriceManagerMb, priceManagerMb, entity, true);

            // 商品表示価格PC関連情報（最高値、最安値、商品表示価格、商品表示値引き前価格）を算出する
            if (isSalePc(isSaleTimeCheck, entity)) {
                calcGoodsDisplayPrice(pricesPc, displayPriceManagerPc, priceManagerPc, entity, false);
            }
            // 2023-renew AddNo5 to here
        }
        ...
        // 2023-renew AddNo5 from here
        // Mobile販売商品 （セール価格用）を設定する
        setResultMb(result, pricesMb, goodsList);
        // 2023-renew AddNo5 to here
    ...
    protected void calcGoodsDisplayPrice(Map<PricesKey, BigDecimal> prices,
                                         ...
                                         GoodsEntity entity, boolean isMbDisplayPrice) {
        // 商品表示価格関連情報（最高値、最安値、商品表示価格、商品表示値引き前価格）を算出する
        BigDecimal price = isMbDisplayPrice ? entity.getPreDiscountPriceLow() : entity.getGoodsPriceInTaxLow();
        BigDecimal preDiscountPrice = isMbDisplayPrice ? entity.getPreDiscountPriceHight() : entity.getGoodsPriceInTaxHight();
        if (price == null) {
            return;
        }
        ...
        if (preDiscountPrice == null) {
            ...
            if (BigDecimal.ZERO.equals(prices.get(PricesKey.MAX)) || preDiscountPrice.compareTo(prices.get(PricesKey.MAX)) > 0) {
                prices.put(PricesKey.MAX, preDiscountPrice);
            }
            // 商品最安値の算出
            if (preDiscountPrice.compareTo(prices.get(PricesKey.MIN)) < 0) {
                prices.put(PricesKey.MIN, preDiscountPrice);
            }
    ...
    // 2023-renew AddNo5 from here
    /**
     * Mobile販売商品の情報を設定する<br/>
     *
     * @param result                算出結果
     * @param pricesMb              価格判定マップ
     * @param goodsList             List<GoodsDto>
     */
    protected void setResultMb(Map<Key, Object> result,
                               Map<PricesKey, BigDecimal> pricesMb,
                               List<GoodsDto> goodsList) {
        // 商品グループ最高値MBを設定
        result.put(Key.MAX_PRICE_MB, pricesMb.get(PricesKey.MAX));
        // 商品グループ最安値MBを設定
        result.put(Key.MIN_PRICE_MB, pricesMb.get(PricesKey.MIN));

        result.put(Key.HAS_SALE_PRICE, goodsList.stream().map(goodsDto -> goodsDto.getGoodsEntity().getPreDiscountPriceHight())
                                                .anyMatch(Objects::nonNull)
                                       && goodsList.stream().map(goodsDto -> goodsDto.getGoodsEntity().getPreDiscountPriceLow())
                                                   .anyMatch(Objects::nonNull));
    }
    // 2023-renew AddNo5 to here
    ...
    protected BigDecimal getDisplayPrediscountPriceValue(Map<PricesKey, BigDecimal> prices,
                                                         ...
                                                         Map<PriceManagerKey, Integer> priceManager) {
        if (prices.get(PricesKey.MIN_PREDISCOUNT) != null &&
            prices.get(PricesKey.MIN).compareTo(prices.get(PricesKey.MIN_PREDISCOUNT)) < 0) {

...
import jp.co.hankyuhanshin.itec.hitmall.batch.offline.clear.dao.FootPrintBatchDao;
...
/**
 * あしあと商品レコードクリアロジック実装クラス
 *
 * @author kk4625
 * @version $Revision:$
 */
@Component
public class FootPrintClearLogicImpl extends AbstractRecordClearLogic implements FootPrintClearLogic {

    /**
     * あしあと商品Dao
     */
    private final FootPrintBatchDao footPrintBatchDao;

    @Autowired
    public FootPrintClearLogicImpl(FootPrintBatchDao footPrintBatchDao) {
        this.footPrintBatchDao = footPrintBatchDao;
    }
    ...
    public Map<String, String> execute(final Integer shopSeq, final Integer specifiedDays) {
        ...
        try {
            ...
            // あしあとレコード総件数取得
            recordCount = footPrintBatchDao.getFootPrintRecordCount(shopSeq);
            // あしあと削除対象件数取得
            if (recordCount > 0) {
                deleteCount = footPrintBatchDao.deleteFootPrintClearRecord(shopSeq, specifiedDaysStr);
            ...
            // 異常終了結果Map作成
            LOGGER.warn("あしあとTBLクリア処理中に異常終了しました：", e);

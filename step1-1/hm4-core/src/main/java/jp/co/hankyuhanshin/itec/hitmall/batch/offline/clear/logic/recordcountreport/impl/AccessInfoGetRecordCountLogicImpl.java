...
import jp.co.hankyuhanshin.itec.hitmall.batch.offline.clear.dao.AccessInfoBatchDao;
...
/**
 * アクセス情報レコードクリアロジック実装クラス
 *
 * @author kk4625
 * @version $Revision:$
 */
@Component
public class AccessInfoGetRecordCountLogicImpl extends AbstractRecordCountReportLogic
                implements AccessInfoGetRecordCountLogic {

    /**
     * アクセス情報Dao
     */
    private final AccessInfoBatchDao accessInfoBatchDao;

    @Autowired
    public AccessInfoGetRecordCountLogicImpl(AccessInfoBatchDao accessInfoBatchDao) {
        this.accessInfoBatchDao = accessInfoBatchDao;
    }
    ...
    public Map<String, String> execute(final Integer shopSeq, final Integer specifiedDays) {
        ...
        try {
            ...
            // アクセス情報レコード総件数取得
            recordCountAll = accessInfoBatchDao.getAccessInfoRecordAllCount(shopSeq);
            // アクセス情報削除対象件数取得
            if (recordCountAll > 0) {
                recordCount = accessInfoBatchDao.getAccessInfoRecordCount(shopSeq, specifiedDaysStr);
            ...
            // 異常終了結果Map作成
            LOGGER.warn("アクセス情報TBLクリア処理中に異常終了しました：", e);

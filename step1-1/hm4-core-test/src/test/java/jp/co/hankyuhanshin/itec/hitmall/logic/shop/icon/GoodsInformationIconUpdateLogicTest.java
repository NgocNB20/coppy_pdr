...
public class GoodsInformationIconUpdateLogicTest {

    @Autowired
    GoodsInformationIconUpdateLogic logic;
    ...
    public void executeTest() {
        ...
        // モック設定
        doReturn(result).when(dto).update(any(GoodsInformationIconEntity.class));

        // 試験実行
        int actual = logic.execute(entity);

        // 戻り値及び呼び出し検証
        verify(dto, times(1)).update(any(GoodsInformationIconEntity.class));

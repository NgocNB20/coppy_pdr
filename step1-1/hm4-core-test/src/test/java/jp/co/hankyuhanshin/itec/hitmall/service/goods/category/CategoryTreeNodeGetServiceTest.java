...
public class CategoryTreeNodeGetServiceTest {

    @Autowired
    CategoryTreeNodeGetService service;

    @MockBean
    private AllNodeCategorySearchLogic logic;

    @Test
    @Order(1)
    public void executeForAllNode() {
        ...
        // モック設定
        doReturn(categoryDetailsDtoList).when(logic).execute(any(String.class));

        // 試験実行
        CategoryDto actual = service.executeForAllNode(categoryId);

...
public class TradedCardLogicTest {
    ...
    public void execute() throws PaymentException {
        ...
        // 想定値設定
        TradedCardOutput result = new TradedCardOutput();
        ...
        doReturn(result).when(paymentClient).doTradedCard(any(HmTradedCardInput.class));
        ...
        TradedCardOutput actual = tradedCardLogic.execute(dto, "test");

        // 戻り値及び呼び出し検証
        verify(mulPayUtility, times(1)).createPaymentMemberId(any());
        verify(paymentClient, times(1)).doTradedCard(any(HmTradedCardInput.class));

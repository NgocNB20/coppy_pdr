...
public class InquiryGroupUpdateServiceTest {

    @Autowired
    InquiryGroupUpdateService service;

    @MockBean
    private InquiryGroupUpdateLogic inquiryGroupUpdateLogic;
    ...
    public void execute1Test() {
        ...
        // モック設定
        doReturn(result).when(inquiryGroupUpdateLogic).execute(any(InquiryGroupEntity.class));

        // 試験実行
        int actual = service.execute(entity);

        // 戻り値及び呼び出し検証
        verify(inquiryGroupUpdateLogic, times(1)).execute(any(InquiryGroupEntity.class));

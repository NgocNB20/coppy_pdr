...
public class AddressBookUpdateLogicTest {

    @Autowired
    AddressBookUpdateLogic logic;
    ...
    public void executeTest() {
        ...
        // モック設定
        doReturn(result).when(dao).update(any(AddressBookEntity.class));

        // 試験実行
        int actual = logic.execute(addressBookEntity);

        // 戻り値及び呼び出し検証
        verify(dao, times(1)).update(any(AddressBookEntity.class));

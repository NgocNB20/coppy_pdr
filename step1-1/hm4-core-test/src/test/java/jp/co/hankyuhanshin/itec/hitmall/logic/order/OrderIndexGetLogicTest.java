...
public class OrderIndexGetLogicTest {

    @Autowired
    OrderIndexGetLogic logic;
    ...
    public void execute() {
        ...
        // 想定値設定
        Integer orderSeq = 1;
        Integer orderVersionNo = 2;
        OrderIndexEntity result = new OrderIndexEntity();

        // モック設定
        doReturn(result).when(dao).getEntity(any(Integer.class), any(Integer.class));

        // 試験実行
        OrderIndexEntity actual = logic.execute(orderSeq, orderVersionNo);

...
public class OrderSettlementGetLogicTest {

    @Autowired
    OrderSettlementGetLogic logic;
    ...
    public void execute() {
        ...
        // 想定値設定
        Integer orderSeq = 1;
        Integer orderSettlementVersionNo = 1;
        OrderSettlementEntity result = new OrderSettlementEntity();

        // モック設定
        doReturn(result).when(dao).getEntity(any(Integer.class), any(Integer.class));

        // 試験実行
        OrderSettlementEntity actual = logic.execute(orderSeq, orderSettlementVersionNo);

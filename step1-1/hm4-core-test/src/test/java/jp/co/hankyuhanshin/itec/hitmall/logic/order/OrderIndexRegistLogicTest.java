...
public class OrderIndexRegistLogicTest {

    @Autowired
    OrderIndexRegistLogic logic;
    ...
    public void execute() {
        ...
        // モック設定
        doReturn(result).when(dao).insert(any(OrderIndexEntity.class));

        // 試験実行
        int actual = logic.execute(orderIndexEntity);

        // 戻り値及び呼び出し検証
        verify(dao, times(1)).insert(any(OrderIndexEntity.class));

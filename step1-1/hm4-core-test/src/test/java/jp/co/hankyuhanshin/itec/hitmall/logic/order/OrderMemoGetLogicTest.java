...
public class OrderMemoGetLogicTest {

    @Autowired
    OrderMemoGetLogic logic;
    ...
    public void execute() {
        ...
        // 想定値設定
        Integer orderSeq = 1;
        Integer orderMemoVersionNo = 1;
        OrderMemoEntity result = new OrderMemoEntity();

        // モック設定
        doReturn(result).when(dao).getEntity(any(Integer.class), any(Integer.class));

        // 試験実行
        OrderMemoEntity actual = logic.execute(orderSeq, orderMemoVersionNo);

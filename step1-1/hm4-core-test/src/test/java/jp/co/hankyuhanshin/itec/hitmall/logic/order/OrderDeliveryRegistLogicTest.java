...
public class OrderDeliveryRegistLogicTest {

    @Autowired
    OrderDeliveryRegistLogic logic;
    ...
    public void execute() {
        ...
        // モック設定
        doReturn(result).when(dao).insert(any(OrderDeliveryEntity.class));

        // 試験実行
        int actual = logic.execute(orderDeliveryEntity);

        // 戻り値及び呼び出し検証
        verify(dao, times(1)).insert(any(OrderDeliveryEntity.class));

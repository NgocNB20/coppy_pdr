...
public class NewsViewableMemberDeleteLogicImplTest {

    @Autowired
    NewsViewableMemberDeleteLogic logic;
    ...
    public void executeTest() {
        ...
        // モック設定
        doReturn(result).when(dao).deleteByNewsSeq(newsSeq);

        // 試験実行
        int actual = logic.execute(newsSeq);

        // 戻り値及び呼び出し検証
        verify(dao, times(1)).deleteByNewsSeq(newsSeq);

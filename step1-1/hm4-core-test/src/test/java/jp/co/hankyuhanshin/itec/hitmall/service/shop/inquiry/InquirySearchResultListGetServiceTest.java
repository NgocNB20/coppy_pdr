...
public class InquirySearchResultListGetServiceTest {

    @Autowired
    InquirySearchResultListGetService service;

    @MockBean
    private InquirySearchResultListGetLogic logic;
    ...
    public void execute1Test() {
        ...
        List<InquirySearchResultDto> listResult = new ArrayList<>();

        // モック設定
        doReturn(listResult).when(logic).execute(any(InquirySearchDaoConditionDto.class));

        // 試験実行
        List<InquirySearchResultDto> actual = service.execute(conditionDto);

        // 戻り値及び呼び出し検証
        verify(logic, times(1)).execute(any(InquirySearchDaoConditionDto.class));

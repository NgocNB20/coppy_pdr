...
public class FreeAreaRegistLogicTest {

    @Autowired
    FreeAreaRegistLogic freeAreaRegistLogic;
    ...
    public void execute() {
        ...
        // モック設定
        doReturn(result).when(freeAreaDao).insert(any(FreeAreaEntity.class));
        ...
        int actual = freeAreaRegistLogic.execute(freeAreaEntity);

        // 戻り値及び呼び出し検証
        verify(freeAreaDao, times(1)).insert(any(FreeAreaEntity.class));

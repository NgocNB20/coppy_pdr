...
/**
 * Logic/Service移行：検索キーワード集計Logic
 * 作成日：2021/03/18
 *
...
public class SearchKeyWordAccordingTotalingListGetLogicTest {

    @Autowired
    SearchKeyWordAccordingTotalingListGetLogic logic;
    ...
    public void executeTest() {
        ...
        // モック設定
        doReturn(resultList).when(dao)
                            .getAccessSearchKeywordTotalList(any(AccessSearchKeywordTotalSearchForConditionDto.class),
                                                             any(SelectOptions.class)
                                                            );

        // 試験実行
        List<AccessSearchKeywordTotalDto> actual = logic.execute(conditionDto);

        // 戻り値及び呼び出し検証
        verify(dao, times(1)).getAccessSearchKeywordTotalList(
                        any(AccessSearchKeywordTotalSearchForConditionDto.class), any(SelectOptions.class));

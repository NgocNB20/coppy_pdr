...
public class PayeasyEntryExecTranLogicTest {
    ...
    public void execute() throws PaymentException {
        ...
        // 想定値設定
        EntryExecTranPayEasyOutput result = new EntryExecTranPayEasyOutput();

        // モック設定
        doReturn(result).when(paymentClient).doEntryExecTranPayEasy(any(HmEntryExecTranPayEasyInput.class));
        ...
        EntryExecTranPayEasyOutput actual = payeasyEntryExecTranLogic.execute(dto);

        // 戻り値及び呼び出し検証
        verify(paymentClient, times(1)).doEntryExecTranPayEasy(any(HmEntryExecTranPayEasyInput.class));

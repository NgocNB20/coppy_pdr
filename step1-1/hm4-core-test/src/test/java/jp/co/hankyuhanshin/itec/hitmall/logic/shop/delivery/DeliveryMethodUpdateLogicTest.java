...
public class DeliveryMethodUpdateLogicTest {
    ...
    public void execute() {
        ...
        // モック設定
        doReturn(result).when(deliveryMethodDao).update(any(DeliveryMethodEntity.class));
        ...
        int actual = deliveryMethodUpdateLogic.execute(deliveryMethodEntity);

        // 戻り値及び呼び出し検証
        verify(deliveryMethodDao, times(1)).update(any(DeliveryMethodEntity.class));

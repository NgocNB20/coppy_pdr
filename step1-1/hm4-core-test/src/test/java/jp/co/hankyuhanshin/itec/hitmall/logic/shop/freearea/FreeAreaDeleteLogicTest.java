...
public class FreeAreaDeleteLogicTest {
    ...
    public void execute() {
        ...
        // モック設定
        doReturn(result).when(freeAreaDao).delete(any(FreeAreaEntity.class));
        ...
        int actual = freeAreaDeleteLogic.execute(freeAreaEntity);

        // 戻り値及び呼び出し検証
        verify(freeAreaDao, times(1)).delete(any(FreeAreaEntity.class));

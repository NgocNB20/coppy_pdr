...
public class GoodsInformationIconRegistLogicTest {

    @Autowired
    GoodsInformationIconRegistLogic logic;
    ...
    public void executeTest() {
        ...
        // モック設定
        doReturn(result).when(dto).getIconSeqNextVal();
        doReturn(result).when(dto).insertAddOrderDisplay(any(GoodsInformationIconEntity.class));

        // 試験実行
        int actual = logic.execute(entity);

        // 戻り値及び呼び出し検証
        verify(dto, times(1)).getIconSeqNextVal();
        verify(dto, times(1)).insertAddOrderDisplay(any(GoodsInformationIconEntity.class));

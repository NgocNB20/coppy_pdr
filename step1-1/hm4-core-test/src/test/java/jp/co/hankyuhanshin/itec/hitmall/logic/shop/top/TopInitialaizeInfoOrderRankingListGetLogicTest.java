...
public class TopInitialaizeInfoOrderRankingListGetLogicTest {

    @Autowired
    TopInitialaizeInfoOrderRankingListGetLogic logic;
    ...
    public void executeTest() {
        ...
        // モック設定
        doReturn(result).when(dao).getOrderRanking(any(Integer.class));

        // 試験実行
        List<GoodsRankingEntity> actual = logic.execute(shopSeq);

        // 戻り値及び呼び出し検証
        verify(dao, times(1)).getOrderRanking(any(Integer.class));

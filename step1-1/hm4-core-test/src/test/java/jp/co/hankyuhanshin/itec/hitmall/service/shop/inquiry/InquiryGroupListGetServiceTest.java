...
public class InquiryGroupListGetServiceTest {

    @Autowired
    InquiryGroupListGetService service;
    ...
    public void execute1Test() {
        ...
        // モック設定
        doReturn(listResult).when(inquiryGroupListGetLogic).execute(any(InquiryGroupSearchForDaoConditionDto.class));

        // 試験実行
        List<InquiryGroupEntity> actual = service.execute();

        // 戻り値及び呼び出し検証
        verify(inquiryGroupListGetLogic, times(1)).execute(any(InquiryGroupSearchForDaoConditionDto.class));

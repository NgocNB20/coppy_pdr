...
public class SettlementMethodRegistServiceTest {

    @Autowired
    SettlementMethodRegistService service;

    @MockBean
    private SettlementMethodMaxOrderDisplayGetLogic settlementMethodMaxOrderDisplayGetLogic;

    @MockBean
    private SettlementMethodRegistLogic settlementMethodRegistLogic;
    ...
    public void executeTest() {
        ...
        settlementMethodEntity.setDeliveryMethodSeq(null);
        ...
        // モック設定
        doReturn(result).when(settlementMethodMaxOrderDisplayGetLogic).execute(any(Integer.class));
        doReturn(result).when(settlementMethodRegistLogic).execute(any(SettlementMethodEntity.class));

        // 試験実行
        int actual = service.execute(settlementMethodDto);

        // 戻り値及び呼び出し検証
        verify(settlementMethodMaxOrderDisplayGetLogic, times(1)).execute(any(Integer.class));
        verify(settlementMethodRegistLogic, times(1)).execute(any(SettlementMethodEntity.class));

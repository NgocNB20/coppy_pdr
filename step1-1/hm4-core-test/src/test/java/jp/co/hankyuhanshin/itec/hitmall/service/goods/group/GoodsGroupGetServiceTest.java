...
public class GoodsGroupGetServiceTest {

    @Autowired
    GoodsGroupGetService service;
    ...
    public void executeTest() {
        ...
        doReturn(result).when(logic)
                        .execute(any(Integer.class), any(String.class), any(), any(HTypeSiteType.class), any(), any());

        // 試験実行
        GoodsGroupDto actual = service.execute(goodsGroupCode);

        // 戻り値及び呼び出し検証
        verify(logic, times(1)).execute(
                        any(Integer.class), any(String.class), any(), any(HTypeSiteType.class), any(), any());

...
public class OrderSummaryRegistLogicTest {

    @Autowired
    OrderSummaryRegistLogic logic;
    ...
    public void execute() {
        ...
        // モック設定
        doReturn(result).when(dao).insert(any(OrderSummaryEntity.class));

        // 試験実行
        int actual = logic.execute(orderSummaryEntity);

        // 戻り値及び呼び出し検証
        verify(dao, times(1)).insert(any(OrderSummaryEntity.class));

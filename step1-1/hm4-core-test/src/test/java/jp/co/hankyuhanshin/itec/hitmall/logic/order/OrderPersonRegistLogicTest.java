...
public class OrderPersonRegistLogicTest {

    @Autowired
    OrderPersonRegistLogic logic;
    ...
    public void execute() {
        ...
        // モック設定
        doReturn(result).when(dao).insert(any(OrderPersonEntity.class));

        // 試験実行
        int actual = logic.execute(orderPersonEntity);

        // 戻り値及び呼び出し検証
        verify(dao, times(1)).insert(any(OrderPersonEntity.class));

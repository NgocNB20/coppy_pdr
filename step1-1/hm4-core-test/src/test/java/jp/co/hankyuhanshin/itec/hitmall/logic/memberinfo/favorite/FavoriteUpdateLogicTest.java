...
public class FavoriteUpdateLogicTest {

    @Autowired
    FavoriteUpdateLogic logic;
    ...
    public void executeTest() {
        ...
        // モック設定
        doReturn(result).when(dao).update(any(FavoriteEntity.class));

        // 試験実行
        int actual = logic.execute(favoriteEntity);

        // 戻り値及び呼び出し検証
        verify(dao, times(1)).update(any(FavoriteEntity.class));

...
public class OrderSettlementRegistLogicTest {

    @Autowired
    OrderSettlementRegistLogic logic;
    ...
    public void execute() {
        ...
        // モック設定
        doReturn(result).when(dao).insert(any(OrderSettlementEntity.class));

        // 試験実行
        int actual = logic.execute(orderSettlementEntity);

        // 戻り値及び呼び出し検証
        verify(dao, times(1)).insert(any(OrderSettlementEntity.class));

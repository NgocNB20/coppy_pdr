...
public final class Utility {
    ...
    /**
     * ストアドプロシージャ呼出文字列を生成する<br/>
     *
     * @param storedName ストアドプロシージャ名
     * @param paramCount パラメータ数（IN,OUTの合計）
     * @return ストアドプロシージャ呼出文字列
     */
    public static String createCallStored(String storedName, int paramCount) {

        StringBuffer buf = new StringBuffer();
        buf.append("{call ");
        buf.append(storedName);
        buf.append("(");
     ...
     */
    public static Response createResponse(Logger log, Result result, String jsonParam, Level logLevel) {

        // 正常終了でない場合は、パラメータをログに出力する
        if (Constant.STORED_OK != result.getResult().getStatus()) {
            log.error(jsonParam);
        }
    ...
    public static void logicException(ApiInfo info, Result result, ResultInfo resultInfo) {
        ...
        // WEB-API内で例外が発生した場合
        resultInfo.setStatus(Constant.STATUS_ERROR);
        resultInfo.setMessage(info.getErrorMsg());
        result.setResult(resultInfo);
    }

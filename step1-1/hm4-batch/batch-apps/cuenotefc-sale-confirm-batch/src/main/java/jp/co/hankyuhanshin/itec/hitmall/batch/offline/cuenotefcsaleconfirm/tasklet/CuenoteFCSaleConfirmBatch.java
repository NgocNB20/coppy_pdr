 ...
 *
 */
package jp.co.hankyuhanshin.itec.hitmall.batch.offline.cuenotefcsaleconfirm.tasklet;
...
/**
 * CuenoteFCセールお知らせメール配信確認バッチ
 *
 * @author takashima
 */
public class CuenoteFCSaleConfirmBatch extends AbstractBatch {

    /**
     * Logger
     */
    private final Logger LOGGER = LoggerFactory.getLogger(CuenoteFCSaleConfirmBatch.class);
     ...
     * 処理名
     */
    private final static String PROCESS_NAME_DELIVERY_CONFIRM = "セールお知らせメール配信状況確認";
     ...
     * コンストラクタ
     */
    public CuenoteFCSaleConfirmBatch(Environment environment) {
    ...
    public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws Exception {
        ...
        if (saleAnnounceMailEntityList.size() == 0) {
            ...
            sendAdminMailLogic.execute(mailMessageDetail, PROCESS_NAME_DELIVERY_CONFIRM,
                                       HTypeMailTemplateType.DELIVERY_CONFIRM, false
                                      );
        ...
        try {
            ...
            // メール送信
            sendAdminMailLogic.execute(mailMessageDetail, PROCESS_NAME_DELIVERY_CONFIRM,
                                       HTypeMailTemplateType.DELIVERY_CONFIRM_ERROR, true
                                      );
            executionContext.put(
                            BatchExitMessageUtil.exitMsg, exitMessageUtil.convertObjectToJson(
                                            new BatchExitMessageDto(String.valueOf(batchProcessTotalCount),
